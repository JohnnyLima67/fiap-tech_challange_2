  ...
  "node_modules/supertest/lib/test.js"(exports2, module2) {
    ...
    var Test = class extends Request {
      ...
      /*
        ...
        */
      bearer(token2) {
        this.set("Authorization", `Bearer ${token2}`);
        return this;
      }
      /*
        * Adds a set Authorization Bearer
...
async function create(request2, reply) {
...
async function findById(request2, reply) {
...
async function findAll(request2, reply) {
  try {
    const postRepository = new PostRepository();
    ...
    const { page = 1, limit = 10 } = querySchema.parse(request2.query);
...
async function updateById(request2, reply) {
  ...
  const { post_title, post_content, post_description, author } = bodySchema.parse(request2.body);
  try {
    const postRepository = new PostRepository();
    const findByIdUseCase = new FindByIdUseCase(postRepository);
...
async function deletePost(request2, reply) {
...
async function findByKeywords(request2, reply) {
  ...
  const { keywords, page = 1, limit = 10 } = querySchema.parse(request2.query);
  try {
    const postRepository = new PostRepository();
...
async function create2(request2, reply) {
  ...
  const { role_name, description } = registerBodySchema.parse(request2.body);
  try {
...
async function updateById2(request2, reply) {
  ...
  const { role_name, description } = bodySchema.parse(request2.body);
  try {
    const roleRepository = new RoleRepository();
...
async function deleteRole(request2, reply) {
...
async function create3(request2, reply) {
  ...
  const { username, password, full_name, role_id } = registerBodySchema.parse(request2.body);
...
async function findById2(request2, reply) {
...
async function findAll2(request2, reply) {
  try {
    const userRepository = new UserRepository();
...
async function updateById3(request2, reply) {
  ...
  const { username, password, full_name, role_id } = bodySchema.parse(request2.body);
  try {
    const userRepository = new UserRepository();
    const findByIdUseCase = new FindByIdUseCase3(userRepository);
...
async function findByUsername(request2, reply) {
  const querySchema = import_zod16.z.object({
    username: import_zod16.z.string().min(1).max(50)
  });
  const { username } = querySchema.parse(request2.query);
  try {
    const userRepository = new UserRepository();
...
async function signin(request2, reply) {
  ...
  const { username, password } = registerBodySchema.parse(request2.body);
  ...
  const token2 = await reply.jwtSign({ username });
  return reply.status(200).send({ token: token2, user: { username: user.username, full_name: user.full_name } });
}
...
async function validateJwt(request2, reply) {
  try {
    const routeFreelist = [
      "POST-/users",
      "POST-/users/signin",
...
beforeAll(async () => {
  ...
  await app.ready();
  const res = await (0, import_supertest.default)(`http://localhost:${env.PORT}`).post("/users/signin").send({
...
describe("POST /posts", () => {
  it("deve criar um post com sucesso", async () => {
    const res = await (0, import_supertest.default)(`http://localhost:${env.PORT}`).post("/posts").set("Authorization", `Bearer ${token}`).send({
      post_title: "T\xEDtulo de Teste",
      post_description: "Descri\xE7\xE3o breve",
      post_content: "Conte\xFAdo completo",
      author: "Autor da postagem"
    });
    expect(res.statusCode).toBe(201);
  });
  it("deve retornar erro ao criar post sem t\xEDtulo", async () => {
    const res = await (0, import_supertest.default)(`http://localhost:${env.PORT}`).post("/posts").set("Authorization", `Bearer ${token}`).send({
      post_description: "Descri\xE7\xE3o breve",
      post_content: "Conte\xFAdo completo",
      author: "Autor da postagem"
...
describe("GET /posts", () => {
  it("deve retornar lista de posts criados", async () => {
    const res = await (0, import_supertest.default)(`http://localhost:${env.PORT}`).get("/posts").set("Authorization", `Bearer ${token}`);
...
async function waitForDatabaseReady(retries = 10, interval = 500) {
