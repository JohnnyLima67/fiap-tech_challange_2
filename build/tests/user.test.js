...
describe("POST /users", () => {
  ...
  it("deve criar um user com sucesso", async () => {
    const res = await (0, import_supertest.default)(`http://localhost:${env.PORT}`).post("/users").set("Authorization", `Bearer ${token}`).send({
      username: "User_Teste",
      password: "12345678",
  ...
  it("deve deletar um user com sucesso", async () => {
    const res = await (0, import_supertest.default)(`http://localhost:${env.PORT}`).delete(`/users/${novoId}`).set("Authorization", `Bearer ${token}`);
